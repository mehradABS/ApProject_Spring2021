...
public class SlowBall extends Prize {
    private static final int Prize_Id= 6;
    public SlowBall(){}
    SlowBall(int YVelocity, ImageIcon icon, int x, int y,
    ...
    public void usePrize() {
        if(GotPrizeNotContains(6)){
            for (MyBall ball:balls) {
                try {
                    ball.changVelocity(false);
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
    ...
    public Prize prizeBuilder(int YVelocity, ImageIcon icon,
        ...
        return new SlowBall( YVelocity,  icon,  x,  y,
         board,  balls);
    }
}
